...
function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white">
      <header className="sticky top-0 z-20 border-b border-white/[0.07] bg-black/80 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h1 className="text-sm font-semibold text-white">Reporting Dashboard</h1>
            </div>
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="flex items-center gap-2 rounded-xl border border-white/[0.08] bg-white/[0.04] px-3 py-2 text-xs font-medium text-white/60 transition hover:bg-white/[0.08] hover:text-white disabled:opacity-50"
          >
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
        <section className="relative mb-6 overflow-hidden rounded-3xl border border-white/[0.08] bg-[#0a0a0a] p-6 sm:p-8">
        ...
        <section className="mb-6 grid gap-6 lg:grid-cols-2">
          <div className="rounded-3xl border border-white/[0.07] bg-[#0a0a0a] p-6">
            <div className="mb-6 flex items-center justify-between">
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-white/25">Distribution</p>
                <h3 className="mt-1 text-lg font-semibold">Difficulty balance</h3>
              </div>
          ...
          <div className="rounded-3xl border border-white/[0.07] bg-[#0a0a0a] p-6">
            <div className="mb-6 flex items-center justify-between">
              <div>
                ...
                <h3 className="mt-1 text-lg font-semibold">Topic representation</h3>
              </div>
              ...
              {TOPICS.map((topic) => {
                ...
                return (
                  <div key={topic} className="flex flex-col items-center gap-3">
                    ...
                    <div className="flex h-40 w-full items-end rounded-xl bg-white/[0.03] p-1">
                      <div
                        className="w-full rounded-lg bg-gradient-to-t from-indigo-600 to-cyan-400 transition-all duration-700"
                        style={{ height: `${height}px` }}
                      />
                    </div>
                    <span className="max-w-full truncate text-[9px] font-semibold uppercase tracking-wider text-white/25">
                      {formatTopic(topic)}
                    </span>
                  </div>
                );
        ...
        <section className="mb-6 grid gap-6 xl:grid-cols-[1fr_340px]">
          <div className="overflow-hidden rounded-3xl border border-white/[0.07] bg-[#0a0a0a]">
            <div className="flex flex-col gap-4 border-b border-white/[0.06] p-5 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-white/25">Catalog report</p>
                <h3 className="mt-1 text-lg font-semibold">Problem inventory</h3>
              </div>
              <label className="flex items-center gap-2 rounded-xl border border-white/[0.08] bg-white/[0.03] px-3 py-2">
                ...
                <input
                  ...
                  className="w-40 bg-transparent text-xs text-white outline-none placeholder:text-white/20"
                />
              </label>
            </div>
              ...
              {filteredProblems.length ? filteredProblems.map((problem, index) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-[10px] text-white/20">Problem #{index + 1}</p>
                  </div>
                  <span className="rounded-md border border-white/[0.08] px-2 py-1 text-[10px] text-white/35">
                    {formatTopic(problem.tags)}
                  </span>
                  <span className="rounded-md bg-white/[0.04] px-2 py-1 text-[10px] capitalize text-white/45">
                    {problem.difficulty}
                  </span>
                </div>
              )) : (
          ...
          <aside className="rounded-3xl border border-amber-300/15 bg-amber-300/[0.05] p-6">
            ...
            <h3 className="mt-2 text-xl font-semibold">Ready for real user analytics</h3>
            ...
            <div className="mt-6 rounded-xl border border-white/[0.07] bg-black/20 p-3 text-[11px] leading-5 text-white/30">
              Connect a protected admin reporting endpoint to replace this availability panel with live user data.
            </div>
          </aside>
        </section>

        <footer className="flex items-center justify-center gap-2 py-3 text-[10px] uppercase tracking-[0.18em] text-white/15">
          <ShieldCheck size={12} />
